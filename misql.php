...
GROUP_CONCAT(DISTINCT p.Nombre SEPARATOR ', ') AS Personajes,
GROUP_CONCAT(DISTINCT p.Poder SEPARATOR ', ') AS Poderes,
GROUP_CONCAT(DISTINCT g.Nombre SEPARATOR ', ') AS Generos,
GROUP_CONCAT(DISTINCT c.Nombre SEPARATOR ', ') AS Creadores

FROM Animes a

LEFT JOIN Estudios e ON a.EstudioID = e.EstudioID
LEFT JOIN AnimePersonaje ap ON a.AnimeID = ap.AnimeID
LEFT JOIN Personajes p ON ap.PersonajeID = p.PersonajeID
LEFT JOIN AnimeGenero ag ON a.AnimeID = ag.AnimeID
LEFT JOIN Generos g ON ag.GeneroID = g.GeneroID
LEFT JOIN AnimeCreador ac ON a.AnimeID = ac.AnimeID
LEFT JOIN Creadores c ON ac.CreadorID = c.CreadorID
